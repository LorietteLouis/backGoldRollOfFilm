...
const {RoleModel, UserModel} = require('../bdd/sequelize')
...
exports.signUp = (req, res) => {
    ...
    .then(hash=>{
        ...
        return UserModel
            .create(dataUser)
            .then(result =>{
                res.status(201).json({message: `Un critiqueur est arrivé.`})
            }) 
    })
    .catch(error => {
...
exports.login = (req, res) => {
    UserModel.findOne({where:{username: req.body.username}})
        .then(user => {
            if (!user)return res.status(404).json({message:`Ce critiqueur n'existe pas.`})
            bcrypt.compare(req.body.password, user.password)
            .then(isValid => {
                if (isValid) {
                    const token = jwt.sign(
                        {
                          data: { username: req.body.username, id: user.id, role: user.RoleId },
                        },
                        SECRET_KEY,
                        { expiresIn: 60 * 60 }
                      );
